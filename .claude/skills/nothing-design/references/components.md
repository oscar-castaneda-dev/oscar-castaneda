...
- Labels: Space Mono, ALL CAPS. Active: `--title` + dot/underline. Inactive: `--disabled`
...
- Radius: 999px (pill) or 4px (technical). Padding: 4px 12px. Active: `--title` border+text
...
- Active: `--title` bg, `--surface` text (inverted). Inactive: transparent, `--caption`
...
- On: `--title` track, `--surface` thumb. Min touch target: 44px
...
**Segments:** Square-ended blocks, no border-radius. Filled = solid status color. Empty = `--border` (already flips per mode — never hardcode a hex here).
...
- **Bar charts:** Vertical, `--title` fill (white in dark, black in light — never literally white), `--border` remainder. Square ends.
...
**Charts:** Line 1.5–2px `--title`, average dashed 1px `--caption`. Axis labels: Space Mono, `--caption`. Grid: `--border`, horizontal only. No area fill, no legend boxes — label lines directly.
...
- **Modals:** Backdrop `rgba(0,0,0,0.8)` dark / `rgba(0,0,0,0.4)` light — the scrim stays black in both modes, but light mode needs far less of it to separate the dialog from off-white paper. Dialog `--card` + `1px solid --outline` + 16px radius, centered max 480px. Close: `[ X ]` top-right ghost button.
